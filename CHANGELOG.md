...
- **Breaking**: `DslBundle` methods now do not return an `Entity`. It was never
  used and made things more difficult to no gain.
- New `Rule`s (#97, #100, #101, #79)
  - `Rule::Content(f32)`: Like `Rule::Children`, but with the image/text content
    of **container node**
  - `{Leaf,}Rule::AspectRatio(f32)` and `{Leaf,}Rule::ContentAspectRatio`: inherit
    the size as a fraction of the perpendicular size, either a specified value
    or based on the content.
  - **Breaking**: `ScreenRoot` is now a `ContentSized` component, like images,
    sprites and text.
  - **Breaking**: `ContentSized` of images and sprites now work differently. It
    needs to be opted-into explicitly with the `ContentAspectRatio` rule.
  - `Root` rules now support `Children`-based rules.
- Fix parameter substitution in template extras (#99)
